...
app.get('/api/configuracion/resumen', (req, res) => {
  const queries = [
    { key: 'totalProductos',     sql: 'SELECT COUNT(*) AS total FROM productos' },
    { key: 'totalProveedores',   sql: 'SELECT COUNT(*) AS total FROM proveedores' },
    { key: 'totalUsuarios',      sql: 'SELECT COUNT(*) AS total FROM usuarios' },
    { key: 'totalSucursales',    sql: 'SELECT COUNT(*) AS total FROM sucursal' },
    { key: 'productosBajoStock', sql: 'SELECT COUNT(*) AS total FROM productos WHERE Stock_anual <= Stock_minimo' },
    { key: 'totalCategorias',    sql: 'SELECT COUNT(*) AS total FROM categorias' },
    { key: 'totalEntradas',      sql: 'SELECT COUNT(*) AS total FROM entradas' },
  ];
  const resumen = {};
  let pendientes = queries.length;
  queries.forEach(({ key, sql }) => {
    db.query(sql, (err, rows) => {
      resumen[key] = err ? 0 : (rows[0]?.total ?? 0);
      pendientes--;
      if (pendientes === 0) res.json(resumen);
    });
  });
});
